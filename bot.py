...
class PathyPly(Plyable):
    def search_db(self, text):
        ...
        if self.db_by_name.get(text, None) != None:
            return self.db_by_name[text]
    ...
    def clean_nones(value):
        ...
        if isinstance(value, list):
            return [PathyPly.clean_nones(x) for x in value if x is not None and x != '' and x != {} and x != []]
        elif isinstance(value, dict):
            return {
                key: PathyPly.clean_nones(val)
                for key, val in value.items()
                if val is not None and val != '' and val != {} and val != [] and key != 'img' and key != '_id' and key != 'sourceId' and key != 'sort' and key != 'uuid' and not (key == 'description' and len(str(value)) > 1000)
            }
        else:
            return value
    ...
    def __init__(self, channel_id):
        ...
        self.db, self.db_by_name = PathyPly.get_db_items()
...
def add_context(self, message):
    ...
    unpunctuated_message = message.lower().translate(str.maketrans('', '', punctuation_to_remove))
    ...
    for word in unpunctuated_message.split(' '):
        ...
        if self.db_by_name.get(word, None) != None:
            relevant_data = self.db_by_name[word]
